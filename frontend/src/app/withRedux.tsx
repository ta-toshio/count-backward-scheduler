...
const getSerializedState = <S extends Store>(
  state: any,
  { serializeState }: Config<S> = {}
...
const initStore = <S extends Store>({
  makeStore,
  context,
}: InitStoreOptions<S>): S => {
...
export const createWrapper = <S extends Store>(
  makeStore: MakeStore<S>,
  config: Config<S> = {}
) => {
  const makeProps = async ({
    ...
    return {
      ...
      initialState: getIsServer()
        ? getSerializedState<S>(state, config)
        : state,
    }
  }
  ...
  const withRedux = (Component: NextComponentType | App | any) => {
    ...
    return class Wrapper extends React.Component<any, any> {
      ...
      static getInitialProps = hasInitialProps
      ...
      hydrate({ initialState, ...props }: any, context: any) {
      ...
      UNSAFE_componentWillReceiveProps(nextProps: any, nextContext: any) {
        if (
          nextProps?.pageProps?.initialState !==
            this.props?.pageProps?.initialState ||
...
// Legacy
const exportDefault = <S extends Store>(
  makeStore: MakeStore<S>,
  config: Config<S> = {}
) => {
  console.warn(
...
export type GetStaticPropsCallback<S extends Store, P> = (
  store: S
) => GetStaticProps<P>
export type GetServerSidePropsCallback<S extends Store, P> = (
  store: S
) => GetServerSideProps<P>
export type PageCallback<S extends Store, P> = (
  store: S
) => GetInitialPageProps<P>
export type AppCallback<S extends Store, P> = (
  store: S
) => GetInitialAppProps<P>
